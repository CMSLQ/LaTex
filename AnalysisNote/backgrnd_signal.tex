...
The method described by Equation~\ref{formula:NeejFromNemujj} is applied assuming 100 pb$^{-1}$ 
...
($N_{eejj}^{Z}$ from Equation~\ref{formula:NeejFromNemujj}) 
...
\begin{figure}[htb]
  ...
  \end{tabular}
  \caption{\small \sl (a) the $M_{ee}$ distribution for FastSim and FullSim MC events after the baseline selection 
    on electrons and jets described in Section~\ref{sec:eventSelection}, (b) the value of the inverse of $R_{OffZ/AtZ}$ 
    as a function of different $S_{T}$ cuts, obtained from the FastSim MC sample.}
...
Equation~\ref{formula:NeejFromNemujj}; $N_{eejjOffZ}$ is the number of 
...
QCD contribution from the sample generated in the lowest $H_T$ bin of $[100,250]$~GeV. 
Only the three remaining bins will be used in the following considerations.

The approach used is to take a QCD multi-jet sample with two fake electrons and two jets (ffjj sample)
that pass all the kinematic selection criteria (see Section~\ref{sec:eventSelection}), and rescale it to estimate 
the QCD multi-jet contamination in the eejj sample as
...
bin of the QCD multi-jet sample. An $S_{T}$ cut of 460 GeV 
(which is the optimized cut value for LQ mass of 250 GeV) is used in the ffjj selection.
...
from all $H_T$ bins of the QCD multi-jet samples after applying $S_T>460~$GeV.
...
The number of QCD multi-jet events in the eejj sample is estimated as 
...
%This statement is even more true for the selection with larger $S_T$ cut values.
