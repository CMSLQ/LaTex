...
%
\item $\sigma_{i\eta i\eta}$: a variable reflecting the distribution along $\eta$ of the electron shower:
\begin{displaymath}
\sigma_{\eta\eta} = \frac{\sum_i w_i( \eta_i - \bar\eta )^2 \quad}{\sum_i w_i} ,
\end{displaymath}
where $\eta_i$ is the eta position of the $i^{th}$ crystal of a 5x5 square of crystals centered at the seed of the supercluster associated to the electron, $\bar\eta$ is the energy weighted mean $\eta$ of the block, and $w_i$ is the weight given to each crystal as defined by:
\begin{displaymath}
w_i = 4.2 + ln (\frac{E_i}{\bar E})
\end{displaymath}
where $E_i$ is the energy in the $i^{th}$ crystal and $\bar E$ is the average energy of the block.
The values for the endcap are corrected for the different crystal size according to a linear correction.
...
\begin{figure}
  \begin{center}
  \begin{tabular}{cc}
    \resizebox{8.2cm}{!}{\includegraphics{plots/electronStudies/Eff_vs_Eta_recoIDIsoEle_pT30_eta2.5_LQ400.eps}} &
    \resizebox{8.2cm}{!}{\includegraphics{plots/electronStudies/Eff_vs_pT_recoIDIsoEle_pT30_eta2.5_LQ400.eps}} \\
    \end{tabular}
