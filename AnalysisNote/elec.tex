...
Recentely HEEP ID and Isolation criteria have been improved
...
\begin{figure}
 ...
 $p_{T}>30$ GeV and $|\eta|<2.5$ if $\Delta$R between them is < 0.02. Reconstructed electrons are associated to the 
...
Table~4
%\ref{tab:ElecEffAcc} 
shows the electron acceptance $A$ for generated $P_{T}$> 30 GeV and $|\eta|<2.5$, the 
reconstruction efficiency within the acceptance region ($\varepsilon^{reco}$), and the overall reconstruction efficiency 
($A \times \varepsilon^{reco}$) for single electrons from LQ decays for a 400 GeV LQ mass.
The overall reconstruction efficiency on a single electron, including acceptance, is $= approx XX\%$.
Fig \ref{fig:elecEffFV} shows the reconstruction efficiency of electrons
for LQ mass of 400 GeV as a function of $\eta$. 
...
geometrical acceptance of the inner traking system, which ends at $|\eta| = 2.5$.
