...
The specific values of the kinematic cuts have been determined using the cut-optimization procedure
...
The $S_T$ cut was used by the {\rm D0} collaboration in 
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \caption{\small \sl Left: invariant mass of the electron pair, $M_{ee}$. 
	     ...
	     the total signal + background distribution, and assuming 100 pb$^{-1}$ of data.
	     }
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \caption{\small \sl Distribution of the invariant mass, $M_{ej}$, 
      ...
      All histograms (two entries per event) are summed on top of each other. 
      Black dots indicate pseudo data randomly generated according to 
      the total signal + background distribution, and assuming 100 pb$^{-1}$ of data.
...
The overall signal selection efficiency is around 35--65\% 
for the LQ masses investigated. 
...
Signal selection efficiencies are also reported for different $LQ$ masses;
the fractional statistical uncertainty on the signal efficiencies is less than 1\%.
...
A scan of the 8-dimensional parameter space is performed using a fast procedure in terms of CPU timing, 
