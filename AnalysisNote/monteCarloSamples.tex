...
\begin{itemize}
%
\item $t\bar{t}$+jet events (for N$<$5), generated using MADGRAPH \cite{Mangano:2002ea} 
%with a LO-NLO K correction factor of 1.85 applied to the LQ cross section;\footnote{The K factor of 1.85 applied to ttbar events is a default in the software provided by CMS for these data samples.  It is from a MCFM NLO vs LO study.}
%
\item $Z/\gamma*$ + N jet events (for N$<$6), generated using MADGRAPH;  
%
\item $W$ + N jet events (for N$<$6), generated using MADGRAPH with $W$ decaying into leptons; % $0< P_{T}^{W} < 300 $ GeV (FIXME)
%
