...
%A more detailed description of the calculations of these numbers can be found in ~\cite{PhysRevD.71.057503}.

%  \begin{table}[htb]
%    \caption{\small \sl Cross Section and K-factors using CTEQ6L1 and CTEQ6M parton densities for LQ productions at a variety of LQ masses\cite{PhysRevD.71.057503} }
%    \label{tab:CTEQ6}
%    \begin{center}
...
\begin{itemize}
%
\item $t\bar{t}$ + jets events, generated using MADGRAPH~\cite{Alwall:2007st}, inclusive production (all decays); 
...
%
\item $Z/\gamma*$ + N jets events (with $N_{jet}\le$4), generated using MADGRAPH, $Z$ decaying into charged leptons;  
%
\item $W$ + N jets events (with $N_{jet}\le$4), generated using MADGRAPH, $W$ decaying into leptons; 
...
\item $VV$ + jets events ($VV$ = $WW$, $WZ$ or $ZZ$), generated with MADGRAPH, 
...
Figure~\ref{fig:genPlots} shows the $p_T$ and $\eta$ distributions at generator level 
of the electrons from the LQ decay. 
Quarks from the LQ decay have almost identical $p_T$ and $\eta$ distributions.
\begin{figure}
  \begin{center}
  \begin{tabular}{cc}
  a.
  \resizebox{7.5cm}{!}{\includegraphics{plots/generatorLevel/GenElepT.eps}} &
  b.
  \resizebox{7.5cm}{!}{\includegraphics{plots/generatorLevel/GenEleEta.eps}} \\
  \end{tabular}
