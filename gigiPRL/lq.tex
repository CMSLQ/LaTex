...
using 
events containing two electrons and two jets from
a data sample corresponding to 50 \ipb~ taken with the CMS
detector during December, 2009 
at the CERN LHC \pp~ collider operating with \sqrts = 10 \tev.
The peak instantaneous luminosity during this run was $2 \times 10^{32}~$cm$^{-2}$s$^{-1}$
and the time between collisions was 75 ns,
corresponding to a  mean number of \pp~ interactions per crossing of 7.
Current limits from the CDF \cite{bib:cdflq}
and D0 experiments \cite{bib:d0lq}
constrain the mass of a first-generation scalar LQ to be $>$ 256 \mgev.

In \pp~ collisions at \sqrts = 10 \tev, LQ's are predominantly pair-produced 
via gluon gluon fusion with a production
cross section that depends on  the strong coupling
constant $\alpha_s$ but is independent of $\lambda$.
The cross section depends on the spin and the mass
of the LQ and has been calculated to
NLO \cite{bib:kramer1}.  The uncertainty on the cross section has been
estimated to be 10\%, and 
is dominated by uncertainties on the parton distribution functions
(PDF's). \cite{bib:kramer2}.

The CMS detector \cite{bib:cmsjinst} consists of a large-bore
superconducting solenoid with a 3.8 T central field 
...
reconstructions of vertices and of the momenta of charged particles with 
...
that makes
a decision for every beam crossing (L1).  The information from all subsystems
...
Events used in this analysis are required to pass a L1 trigger that requires a
cluster of energy in the electromagnetic calorimeter with \et $>$ 10
\pgev~ that passes a loose requirement on the ratio of hadronic
to electromagnetic energy and shower shape requirements on
the trigger tower in cluster containing the highest energy
in the first level trigger.  The event must also have an electron candidate
with \et $>$
15 \pgev~ reconstructed by the HLT that is well-matched to the L1 EM cluster.

Offline, Electron candidates are required to have 
...
Jets are reconstructed from the calorimeter energies using an 
iterative cone algorithm with a cone size of 0.5.  
The jet energy scale is determined using
...
scalar sum of the magnitudes of the \pt~ of the electrons and jets, be large.
The value of the \st~ requirement depends on the LQ mass and is given
in table~\ref{tab:lqeff}.
The cut value was chosen to optimize the expected
signal significance.
...
\begin{center}
 ...
 550 & XX & XX & XX & XX  \\ \hline
  \end{tabular}
    \caption{\st~ requirement,
acceptance (A) for \lqlqbar~ events to pass the geometric and kinematic
requirements of this analysis, acceptance times efficiency (A$\cdot$e) to pass
the electron and jet identification requirements, 
and number of events expected in 50 \ipb, as a function of \lq~ mass }
    \label{tab:lqeff}
\end{center}
\end{table}    
...
electrons and muons, while LQ decays are constrained to only
...
resolutions. 
We observe XXX events in our control sample for an \st~ requirement
...
selection requirements, except we reverse the \mee~ requirement.
The predicted background is then
$$B_{\zee} = N^{data}_{\mee<100} {{N^{MC}_{\mee>100}}\over{N^{MC}_{\mee<100}}}$$
where ${{N^{MC}_{\mee>100}} \ {N^{MC}_{\mee<100}}}=xxx$ and was evaluated
using MC.
We observe XXX events in our control sample for an \st~ requirement
of 460 GeV, for a statistical uncertainty on our background
predicition of XX\%.
...
also agree well. Figure ~\ref{fig:ejmass} shows the electron-jet mass distribution
from data, for the predicted backgrounds and for the signal.
...
Please note that there is not even a hint of a \lq~ signal in this distribution ;) }
\end{figure}
...
Limits on the LQ mass  as a function of $\beta$ are given in Figure ~\ref{fig:finalresult}
